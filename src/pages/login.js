      ...
      <article class="flex flex-col items-center justify-center min-h-[calc(100vh-120px)] max-w-md mx-auto">
        ...
        <form id='loginForm' class="w-full space-y-8">
          ...
          <section class="space-y-3">
            ...
            <input type="email" id="email" name="email" class="w-full px-4 py-4 bg-white border border-[#cbcbcb] rounded-lg focus:outline-none focus:border-[#f56d95] focus:ring-1 focus:ring-[#f56d95] transition-colors" required>
          </section>

          <!-- Password Field -->
          <section class="space-y-3">
            <label for="password" class="block text-[#f56d95] text-base font-medium">Contraseña</label>
            <input type="password" id="password" name="password" class="w-full px-4 py-4 bg-white border border-[#cbcbcb] rounded-lg focus:outline-none focus:border-[#f56d95] focus:ring-1 focus:ring-[#f56d95] transition-colors" required>
          </section>

          <!-- Sign In Button -->
          <button type="submit" class="w-full bg-[#f56d95] text-white py-4 rounded-full text-base font-medium hover:bg-[#e55a87] transition-colors mt-16">
            Iniciar Sesión
          </button>
        </form>
      </article>
      ...
      if (localStorage.getItem("justRegistered") && user.user_id === 1) {
        try {
          await saveAchievedGoal({
            ...
            goal_id: 1 // 👈 ID de "Primer paso solidario" (registro)
          });
        } catch (err) {
          console.warn("Meta de registro ya alcanzada o error:", err.message);
        }
