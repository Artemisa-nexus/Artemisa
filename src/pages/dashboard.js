  ...
  return `
    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-2" data-id="${publication.publication_id}">
      ...
      <div class="flex justify-end space-x-2 text-sm">
        <button class="edit-btn text-[#d08306] hover:underline">Edit</button>
        <button class="delete-btn text-[#ef366d] hover:underline">Delete</button>
      </div>
    </div>
  `;
}
...
//render all the dashboard
      ...
      <main class="flex-1 p-6">
        <div class="max-w-2xl mx-auto space-y-6">
          ...
          <h2 class="text-xl font-semibold text-gray-800">
            Welcome again, <span class="text-[#f56d95]">${user.fullname}</span>
          </h2>

          <!-- Form for the new publication -->
          <form id="new-publication-form" class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-3">
            <textarea 
              ...
              rows="3" 
              placeholder="¿What are you thinking today, ${user.fullname}?">
            </textarea>
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      console.error("Error creating the publication:", err.message);
    }
  });
  ...
  postsContainer.addEventListener("click", async (e) => {
    ...
    if (e.target.classList.contains("delete-btn")) {
      try {
        ...
        console.error("Error deleting the publication:", err.message);
      }
    }
    ...
    if (e.target.classList.contains("edit-btn")) {
      ...
      const newContent = prompt("Edite the publication:", oldContent);
      ...
      if (newContent && newContent !== oldContent) {
        try {
          ...
          console.error("Error updating the publication:", err.message);
        }
      }
    }
  });
